...
public class UserManager extends CommonManager {
	...
	public static User getUserByDevice(
			CassandraClient cassandraClient, String deviceId) {
		
		String userId = cassandraClient.getColumnValue(DBConstants.INDEX_USER, DBConstants.KEY_DEVICEID, deviceId);
		if (userId == null){
			return null;
		}
		
		Rows<String, String, String> rows = cassandraClient.getMultiRow(DBConstants.USER, userId);
		if (rows == null || rows.getCount() <= 0){
			log.warning("<getUserByDevice> deviceId("+deviceId+") is bind but userId not found");
			return null;
		}
